...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/10/14 11:54:37 by thi-nguy          #+#    #+#             */
/*   Updated: 2021/10/14 11:59:23 by thi-nguy         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (head_command[i].command_line != NULL)
	{
		...
		while (current_token->next != NULL)
		{
			if (current_token->type == redirect_input
				|| current_token->type == redirect_output)
				len = ft_strlen(current_token->value);
			if (current_token->type == redirect_input)
				update_redirection_type(len, &head_command[i].redirect_type,
					&current_token->type, redirect_input);
